...
def get_wave(direction, duration=0.1, amp=1, sample_rate=44100):
    ...
    with sd.OutputStream(samplerate=sample_rate, channels=2) as stream:
        while True:
            ...
            with lock:
                if not running:
                    break
                current_interval = 0.5 * (1-abs(direction))
...
def test_audio_indications():
    ...
    x = threading.Thread(target=play_audio)
    x.start()
...
def test_track():
    ...
    js = k_js.KartJoystickInput()
    try: 
        for i in data.to_numpy(np.int32)[:, 1]:
            js.trigger_update()
            js.load_current_state(k_js.JS_SETUP, k_js.JS_DZ)

            steering =  ((js.steering_angle - i) if js.steering_angle - i  != 0 else 10)
            
            print(i, js.steering_angle, steering)
